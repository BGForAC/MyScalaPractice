...
object MailSystemHelper {
  ...
  def randomPlayerId: Long = MyUtils.getRandom(allPlayersId)
  def randomItemId: Long = MyUtils.getRandom(allItemsID)
  ...
  def actionLoadRandomPlayerMails(): (ListBuffer[Mail], Long) = {
    val playerId = MyUtils.getRandom(allPlayersId)
  ...
  def generateRandomItemIds(offset: Int)(length: Int): Array[Long] = {
    ...
    (for (_ <- 1 to fixedLength) yield MyUtils.getRandom(itemsId)).toArray
  }
  ...
  def readMail(mails: Seq[Mail], playerId: Long): Unit = {
    PlayerService.readMail(PlayerService.getPlayer(playerId), MyUtils.getRandom(mails).getMailId)
  }
  ...
  def collectAttachment(mails: Seq[Mail], playerId: Long): Unit = {
    PlayerService.collectAttachment(PlayerService.getPlayer(playerId), MyUtils.getRandom(mails))
  }
